...
async function main() {
  ...
  const serverInfo = (initialized.body as { result?: { serverInfo?: { name?: string } } }).result
    ?.serverInfo;
  ...
  const tools = (
    (listed.body as { result?: { tools?: Array<{ name: string }> } }).result?.tools ?? []
  )
    .map((tool) => tool.name)
    .sort();
  const expected = [...ASKCHING_TOOL_NAMES].sort();
  if (JSON.stringify(tools) !== JSON.stringify(expected)) {
    throw new Error(`expected tools [${expected.join(', ')}] but got [${tools.join(', ')}]`);
  }
  ...
  if (!structured || (structured.findings?.length ?? 0) < 2) {
    throw new Error(
      `tools/call analyze_trends returned no findings: ${JSON.stringify(called.body)}`,
    );
  }
