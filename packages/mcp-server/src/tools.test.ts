import { createMarketDataSource, type MarketDataSource } from '@askching/shared';
...
describe('discoverYields', () => {
  it('applies defaults and returns separate cited lending and LP rankings', async () => {
    const result = await discoverYields({}, createMarketDataSource({ DEMO_LIVE: '0' }));
  ...
  it('rejects unsupported assets, chains, and bounds before data access', () => {
    ...
    expect(() => DiscoverYieldsInputSchema.parse({ chain: 'sui-mainnet' })).toThrow(
      /ethereum-mainnet/,
    );
    ...
    expect(z.object(DiscoverYieldsCoreSchema.shape).safeParse({ asset: 'WETH' }).success).toBe(
      false,
    );
  });
        ...
        return [
          uniswap!,
          {
            ...
            gaps: [
              {
                venue: 'curve',
                reason: 'Curve data is unavailable from its configured Graph source.',
              },
            ],
          },
        ];
...
describe('analyzeMarkets', () => {
  ...
  it('rejects missing primary objective metrics', () => {
    expect(() =>
      AnalyzeMarketsInputSchema.parse({
...
describe('compareMarkets', () => {
  it('returns a normalized comparison with two fixture citations', async () => {
    ...
    expect(new Set(result.sources.map((source) => source.subgraphId)).size).toBe(2);
  ...
  it('rejects requests with fewer than two protocols before data access', async () => {
    ...
    await expect(
      compareMarkets({ metric: 'usdc_supply_apy', protocols: ['aave-v3'] }, dataSource),
  ...
  it('rejects an unknown metric with a registry hint', async () => {
    ...
    await expect(
      compareMarkets({ metric: 'borrow_tvl', protocols: ['aave-v3', 'compound-v3'] }, dataSource),
  ...
  it('exposes metric/asset validation in the MCP input schema (shape)', () => {
    ...
    const shapeSchema = z.object(exposed).extend({
  ...
  it('exposes the same validation on research_brief and risk_scan shapes', () => {
    ...
    expect(researchShape.safeParse({ metric: 'borrow_tvl', question: 'q' }).success).toBe(false);
    expect(
...
describe('researchBrief', () => {
  ...
  it('honors the requested metric and asset in the brief', async () => {
    ...
    expect(result.brief.keyFigures.every((figure) => figure.metric === 'supply_apy')).toBe(true);
  });
  ...
  it('rejects fewer than two protocols', async () => {
    ...
    await expect(
      researchBrief({ question: 'single protocol', protocols: ['aave-v3'] }, dataSource),
...
describe('analyzeTrends', () => {
  ...
  it('reports a requested protocol without history as an explicit gap', async () => {
    ...
    expect(result.findings.map((finding) => finding.protocol)).toEqual(['aave-v3', 'compound-v3']);
    expect(result.gaps).toEqual([
  ...
  it('rejects fewer than two protocols before data access', async () => {
    ...
    await expect(
      analyzeTrends({ metric: 'supply_apy', protocols: ['aave-v3'], window: '7d' }, dataSource),
    ).rejects.toThrow(/at least 2/);
  });
  ...
  it('exposes window, metric, and asset validation in the MCP shape', () => {
    ...
    expect(exposed.safeParse({ metric: 'borrow_tvl', protocols, window: '7d' }).success).toBe(
      false,
    );
    expect(exposed.safeParse({ metric: 'supply_apy', protocols, window: '90d' }).success).toBe(
      false,
    );
    expect(
      exposed.safeParse({
    ...
    expect(exposed.safeParse({ metric: 'supply_apy', protocols, window: '7d' }).success).toBe(true);
  });
});
...
describe('riskScan', () => {
  it('returns peer-relative findings with an explicit time-series gap', async () => {
    ...
    expect(result.gaps.some((gap) => gap.reason.includes('No time-series data'))).toBe(true);
    expect(
      result.gaps.every(
        (gap) =>
  ...
  it('rejects fewer than two protocols', async () => {
    ...
    await expect(riskScan({ protocols: ['aave-v3'], window: '7d' }, dataSource)).rejects.toThrow(
