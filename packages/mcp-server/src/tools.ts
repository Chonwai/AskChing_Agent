...
const UsdcOnlyFieldSchema = z
  .string()
  .refine((value) => value.toUpperCase() === 'USDC', 'discover_yields supports USDC only');
const EthereumMainnetFieldSchema = z
  .string()
  .refine(
    (value) => value.toLowerCase() === 'ethereum-mainnet',
    'discover_yields supports ethereum-mainnet only',
  );
const StablecoinFieldSchema = z
  .string()
  .refine(
...
export const DiscoverYieldsInputSchema = DiscoverYieldsCoreSchema.transform((value) => ({
  ...
  stablecoins: [
    ...new Set(value.stablecoins.map((stablecoin) => stablecoin.toUpperCase() as 'USDT' | 'DAI')),
  ],
...
export async function discoverYields(
  ...
  if (input.venues.includes('lending')) {
    for (const metric of ['supply_apy', 'utilization', 'tvl'] as const) {
      try {
        lendingObservations.push(
          ...(await dataSource.getObservations(metric, LIVE_PROTOCOLS, input.asset)),
        );
...
export const AnalyzeMarketsInputSchema = AnalyzeMarketsCoreSchema.transform((value) => ({
  ...value,
  ...
  const required =
    value.objective === 'yield_opportunity'
      ? 'supply_apy'
...
export async function analyzeMarkets(
  ...
  for (const metric of input.metrics) {
    try {
      const values = await dataSource.getObservations(metric, input.protocols, input.asset);
...
export const AnalyzeTrendsInputSchema = AnalyzeTrendsCoreSchema.transform((value) => ({
  ...value,
  asset: AssetSymbolSchema.parse(value.asset.toUpperCase()),
...
export const CompareMarketsInputSchema = CompareMarketsCoreSchema.superRefine((value, ctx) => {
  try {
...
export const ResearchBriefInputSchema = ResearchBriefCoreSchema.superRefine((value, ctx) => {
  try {
    resolveMetricId(value.metric);
  } catch (error) {
    ctx.addIssue({
...
export const RiskScanInputSchema = RiskScanCoreSchema.superRefine((value, ctx) => {
  try {
    resolveMetricId(value.metric);
  } catch (error) {
    ctx.addIssue({
...
export async function compareMarkets(
  ...
  const observations = await dataSource.getObservations(metricId, input.protocols, input.asset);
...
export async function researchBrief(
  ...
  if (!input.protocols || input.protocols.length < 2) {
    throw new Error('research_brief requires at least two protocols for a cited comparison');
  }
  const { metricId } = resolveMetricId(input.metric);
...
export async function riskScan(
  ...
  if (protocols.length < 2) {
    throw new Error('risk_scan requires at least two protocols for a peer-relative scan');
  }
  const { metricId } = resolveMetricId(input.metric);
  ...
  for (const asset of input.assets) {
    try {
      const comparison = await compareMarkets({ metric: metricId, asset, protocols }, dataSource);
  ...
  const latest = comparisons.reduce(
    (latest, comparison) => (comparison.asOf > latest ? comparison.asOf : latest),
    comparisons[0]!.asOf,
  );
...
export const GetInfoInputSchema = z
  .object({
    topic: z.enum(['overview', 'tools', 'examples', 'all']).optional().default('all'),
  })
  ...
  tools: [
    {
      ...
      purpose:
        'Ranked cross-protocol comparison of a spot metric (supply_apy, borrow_apy, tvl, utilization).',
      example:
    ...
    },
    {
      ...
      purpose:
        'Transparent yield-opportunity, liquidity-stress, or evidence-quality analysis with calculations and confidence.',
      example:
        'Analyze the best current USDC supply-yield opportunity across Aave V3, Compound V3, and Spark Lend. Show the APY spread calculation, utilization context, and citations.',
    },
    {
      name: 'analyze_trends',
      purpose:
        'Cited 7d / 30d daily history: change, least-squares slope, direction, volatility, min/max.',
      example:
        'How has USDC supply APY trended across Aave V3, Compound V3, and Spark Lend over the last seven days? Cite every data point.',
    },
    {
      name: 'research_brief',
      purpose:
        'Structured cited brief: conclusion, key figures with sources, risks, and a suggested follow-up.',
      example:
        'Prepare a research brief on USDC supply APY across Aave V3, Compound V3, and Spark Lend.',
    },
    {
      name: 'risk_scan',
      purpose: 'Peer-relative spot risk signals and explicit data gaps across protocols.',
      example:
        'Scan Aave V3, Compound V3, and Spark Lend for unusual USDC risk signals and note any data gaps.',
    },
    {
      name: 'discover_yields',
      purpose:
        'Cross-venue USDC yield discovery: separate lending and DEX LP (Uniswap V3, Curve) rankings with formula inputs and risks.',
      example:
        'Where can I earn yield on USDC across lending, Uniswap V3, and Curve? Keep lending and LP rankings separate and do not propose a transaction.',
    },
  ],
