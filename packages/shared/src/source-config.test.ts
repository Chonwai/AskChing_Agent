...
describe("assertLiveProtocols", () => {
  it("accepts only live protocols", () => {
    expect(() =>
      assertLiveProtocols(["aave-v3", "compound-v3", "aave-v2"])
    ).not.toThrow();
  });
  ...
  it("rejects non-live protocols with a live list hint", () => {
    ...
    expect(() =>
      assertLiveProtocols(["aave-v3", "zerolend"])
    ).toThrow(/Protocols not yet live: zerolend\. Live: aave-v3, compound-v3, spark-lend, aave-v2/);
  });
});