...
describe('createDexYieldDataSource', () => {
  it('returns deterministic requested fixtures without fetching or claiming live data', async () => {
    const fetchImpl = vi.fn(() => {
      throw new Error('fixtures must not fetch');
    });
  ...
  it.each([
    ...
    const fetchImpl = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (venue === 'uniswap-v3') {
        ...
        return new Response(JSON.stringify(isPools ? uniswapPoolsPayload() : uniswapPayload()), {
          status: 200,
        });
      }
  ...
  it('requires a Graph key only in live mode', async () => {
    ...
    await expect(
      source.getDexYieldOpportunities({ venues: ['curve'], stablecoins: ['DAI'] }),
