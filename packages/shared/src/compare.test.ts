...
describe('compareObservations', () => {
  it('ranks comparable observations and exposes distinct cited sources with an as-of time', () => {
    ...
    expect(result.rows.map((row) => row.protocol)).toEqual(['aave-v3', 'compound-v3']);
  ...
  it('rejects an observation whose citation is missing a query hash', () => {
    ...
    expect(() => compareObservations([uncited], 'supply_apy')).toThrow(/queryHash/);
  });

  it('rejects a comparison mixing observations across assets', () => {
    expect(() => compareObservations([compound, { ...aave, asset: 'WETH' }], 'supply_apy')).toThrow(
      /same asset/,
    );
  });
  ...
  it('flags utilization as a risk signal rather than a better outcome', () => {
    ...
    const result = compareObservations([utilization, utilizationPeer], 'utilization');

    // Highest utilization ranks first, but the caveat reframes it as risk.
    expect(result.rows.map((row) => row.protocol)).toEqual(['aave-v3', 'compound-v3']);
    expect(result.caveats.some((caveat) => caveat.includes('risk signal'))).toBe(true);
    expect(result.caveats.some((caveat) => caveat.includes('Utilization is ranked'))).toBe(true);
  });
});
