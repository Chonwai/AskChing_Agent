...
function uniswapSnapResponse(
  poolId: string,
  overrides: Partial<UniswapSnapFixture>,
): UniswapSnapResponse {
  const pool =
...
describe('UniswapV3YieldAdapter', () => {
  it('two-phase: queries pools then per-pool snapshots', async () => {
    const snapMap: Record<string, UniswapSnapResponse> = {
      '0x1111111111111111111111111111111111111111': uniswapSnapResponse(
        '0x1111111111111111111111111111111111111111',
        {},
      ),
      '0x2222222222222222222222222222222222222222': uniswapSnapResponse(
    ...
    expect(result.gaps.map((g) => g.poolAddress)).toContain(
  ...
  it('excludes non-stablecoin pool (WETH) from gap list', async () => {
    ...
    expect(result.gaps.map((g) => g.poolAddress)).not.toContain(
  ...
  it('single-pool snapshot failure does not crash other pools', async () => {
    ...
    const fetchImpl = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify(uniswapSnapResponse('0x2222222222222222222222222222222222222222', {})),
        { status: 200 },
      );
    });
    const adapter = new UniswapV3YieldAdapter({
    ...
    expect(
      result.gaps.some((g) => g.poolAddress === '0x1111111111111111111111111111111111111111'),
    ).toBe(true);
  });
  ...
  it('pools query failure throws without leaking credentials', async () => {
    ...
    await expect(adapter.getOpportunities({ stablecoins: ['USDT'] })).rejects.toThrow(
      'Uniswap V3 Graph request failed',
    );
    await expect(adapter.getOpportunities({ stablecoins: ['USDT'] })).rejects.not.toThrow(
      /top-secret/,
    );
  });
  ...
  it('does not leak credentials from HTTP errors', async () => {
    ...
    await expect(adapter.getOpportunities({ stablecoins: ['USDT'] })).rejects.toThrow(
      'Uniswap V3 Graph request failed with HTTP 401',
    );
    await expect(adapter.getOpportunities({ stablecoins: ['USDT'] })).rejects.not.toThrow(
      /my-secret-key/,
    );
...
function curveResponse(): CurveFixtureEnvelope {
  const pool = (id: string, tokens: Array<{ id: string; symbol: string }>) => ({
    id,
    inputTokens: tokens,
  });
      ...
      liquidityPoolDailySnapshots: [
        {
          ...
          timestamp: String(day + 86_399),
          blockNumber: '456',
          dailySupplySideRevenueUSD: '200',
          dailyVolumeUSD: '400000',
          totalValueLockedUSD: '2000000',
          pool: pool('0x5555555555555555555555555555555555555555', [usdc, usdt]),
        },
        {
          id: 'two-old',
          timestamp: String(day - 1),
          blockNumber: '400',
          dailySupplySideRevenueUSD: '999',
          dailyVolumeUSD: '1',
          totalValueLockedUSD: '1',
        ...
        {
          id: 'three-current',
          timestamp: String(day + 86_500),
          blockNumber: '500',
          dailySupplySideRevenueUSD: '999',
          dailyVolumeUSD: '1',
          totalValueLockedUSD: '1',
          pool: pool('0x6666666666666666666666666666666666666666', [usdc, usdt, dai]),
        },
        {
          id: 'three-new',
          timestamp: String(day + 86_399),
          blockNumber: '457',
          dailySupplySideRevenueUSD: '300',
          dailyVolumeUSD: '600000',
        ...
        },
        {
          ...
          pool: pool('0x7777777777777777777777777777777777777777', [usdt, dai]),
        },
        {
          id: 'zero-tvl',
          timestamp: String(day + 86_399),
          ...
          dailyVolumeUSD: '1',
          totalValueLockedUSD: '0',
          pool: pool('0x8888888888888888888888888888888888888888', [usdc, usdt]),
        },
      ],
  ...
  it('matches complete two- and three-token pools and calculates same-day fee APR', async () => {
    const adapter = new CurveYieldAdapter({
      ...
      fetchImpl: vi
        .fn()
        .mockImplementation(
          async () => new Response(JSON.stringify(curveResponse()), { status: 200 }),
        ),
      now: () => new Date('2026-09-11T12:00:00.000Z'),
    });
    ...
    expect(
      result.observations.every((value) => value.windowStart === '2026-09-10T00:00:00.000Z'),
    ).toBe(true);
    expect(result.gaps).toEqual([
      expect.objectContaining({
        poolAddress: '0x8888888888888888888888888888888888888888',
        reason: expect.stringMatching(/positive TVL/),
      }),
    ]);
  ...
  it('does not combine fee revenue and TVL from different daily snapshots', async () => {
    ...
    expect(result.gaps).toEqual([
      expect.objectContaining({
        ...
        reason: expect.stringMatching(/same daily snapshot/),
      }),
    ]);
  });
