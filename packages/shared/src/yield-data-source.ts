...
export function createDexYieldDataSource(
    ...
    return {
      async getDexYieldOpportunities(input) {
        ...
        return selected.map(venue => ({
          venue,
          observations: DEX_YIELD_FIXTURES.filter(observation =>
            observation.venue === venue && matchesStablecoins(observation.tokenSymbols, input.stablecoins)
          ),
          gaps: []
        }));
      }
  ...
  return {
    async getDexYieldOpportunities(input) {
      ...
      const selected = [...new Set(input.venues)];
      const settled = await Promise.allSettled(selected.map(venue => {
        ...
        const adapter = venue === "uniswap-v3"
      ...
      return settled.map((result, index) => result.status === "fulfilled"
        ...
        : {
            ...
            observations: [],
            gaps: [{ venue: selected[index]!, reason: `${displayVenue(selected[index]!)} data is unavailable from its configured Graph source.` }]
          }
      );
    }
  };
