...
export function normalizeYieldDiscovery(input: NormalizeYieldDiscoveryInput): DiscoverYieldsResult {
  ...
  if (
    lendingRaw.some((value) => value.asset !== 'USDC') ||
    dexRaw.some((value) => value.asset !== 'USDC')
  ) {
  ...
  const hasBothDexVenues =
    commonWindow !== undefined &&
  ...
  if (
    !hasBothDexVenues &&
    validDex.length > 0 &&
    !gaps.some((value) => /cross-DEX|common complete UTC day/.test(value.reason))
  ) {
    gaps.push({
      reason:
        'A cross-DEX winner requires qualifying Uniswap V3 and Curve evidence on a common complete UTC day.',
    });
  }
...
function normalizeLending(
  ...
  const supply = observations.filter(
    (value) =>
  ...
  if (new Set(supply.map((value) => value.subgraphId)).size < 2) {
    if (supply.length > 0)
      gaps.push({
        venue: 'lending',
        reason: 'Lending ranking requires at least two distinct cited subgraph sources.',
      });
    return [];
  }
  return supply
    .flatMap((rate) => {
      const context = observations.filter(
        (value) =>
      ...
      if (context.length === 0) {
        gaps.push({
          venue: 'lending',
          reason: `${rate.protocol} lacks a second cited context metric.`,
        });
        return [];
      }
      ...
      return [
        {
          ...
          citations,
          riskFlags: [
            'spot_rate_variable' as const,
            'stablecoin_depeg' as const,
            'smart_contract' as const,
          ],
          caveats: [
            'Spot supply APY can change and is not a forecast or financial recommendation.',
          ],
        },
      ];
    })
  ...
  return observations.filter((value) => {
    ...
    if (Math.abs(value.estimatedFeeApr - expectedApr) > tolerance) {
      gaps.push({
        ...
        reason: 'Reported fee APR does not match its cited daily fee and TVL inputs.',
      });
      return false;
    }
    if (Date.parse(value.windowEnd) - Date.parse(value.windowStart) !== DAY_MS) {
      gaps.push({
        venue: value.venue,
        poolAddress: value.poolAddress,
        reason: 'DEX evidence must cover exactly one complete UTC day.',
      });
      return false;
    }
    return true;
...
function selectDexWindows(
  ...
  if (common) {
    return {
      observations: observations.filter((value) => windowKey(value) === common),
      commonWindow: common,
    };
  }
  ...
  if (presentVenues.length === 2) {
    gaps.push({
      reason:
        'Uniswap V3 and Curve have no common complete UTC day; venue-local results are not a cross-DEX comparison.',
    });
  }
  const latestByVenue = new Map(
    presentVenues.map((venue) => [venue, [...windows.get(venue)!].sort().at(-1)!]),
  );
  return {
    observations: observations.filter(
      (value) => latestByVenue.get(value.venue) === windowKey(value),
    ),
  };
}

function toDexOpportunity(value: DexYieldObservation, rank: number): DexLpYieldOpportunity {
  return {
    ...value,
    category: 'dex_lp',
    rank,
    ...
    riskFlags: [
      ...
      ...(value.venue === 'uniswap-v3'
        ? ['concentrated_liquidity' as const, 'position_range_dependent' as const]
        : []),
      ...(value.venue === 'curve' && value.tokenSymbols.length > 2
        ? ['multi_asset_pool' as const]
        : []),
    ],
    caveats:
      value.venue === 'uniswap-v3'
