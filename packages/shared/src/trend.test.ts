...
describe("computeTrendStats", () => {
  it("requires at least two points", () => {
    expect(() =>
      computeTrendStats(MARKET_HISTORY_FIXTURES.filter(
        (point) => point.metric === "supply_apy" && point.protocol === "aave-v3" && point.days === 6
      ))
  ...
  it("summarizes a falling series", () => {
    const stats = computeTrendStats(
      fixtureSeries("supply_apy", ["compound-v3"])[0]!.points
    );
  ...
  it("labels a near-flat series as flat", () => {
    const stats = computeTrendStats(
      fixtureSeries("supply_apy", ["spark-lend"])[0]!.points
    );
...
describe("analyzeTrendSeries", () => {
  it("produces one cited finding per protocol with high confidence on full fixture coverage", () => {
    ...
    for (const finding of result.findings) {
      ...
      expect(finding.points.map((point) => point.days)).toEqual([
        0, 1, 2, 3, 4, 5, 6
      ]);
      expect(
    ...
    const aave = result.findings.find(
      (finding) => finding.protocol === "aave-v3"
    )!;
    ...
    const compound = result.findings.find(
      (finding) => finding.protocol === "compound-v3"
    )!;
    expect(compound.stats.direction).toBe("falling");

    const spark = result.findings.find(
      (finding) => finding.protocol === "spark-lend"
    )!;
  ...
  it("reports a wider window as an explicit gap instead of padding the series", () => {
    ...
    expect(
      result.findings.every((finding) => finding.confidence === "medium")
    ).toBe(true);
  });
  ...
  it("drops to medium confidence when a series has fewer than five points", () => {
    ...
    expect(result.findings.every((finding) => finding.points.length === 4)).toBe(
      true
    );
    expect(
      result.findings.every((finding) => finding.confidence === "medium")
  ...
  it("fails closed when no series survives at all", () => {
    expect(() =>
      analyzeFixture("supply_apy", { series: [], gaps: [] })
    ).toThrow(/got 0 from 0 source\(s\).*none reported/s);
  });
