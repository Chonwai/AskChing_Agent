...
import { GET_MARKET_HISTORY_QUERY, GET_MARKETS_QUERY, GraphGatewayClient } from './graph-client.js';
...
describe('GraphGatewayClient.getMarketObservation', () => {
  it('extracts the max LENDER/VARIABLE rate for supply_apy/USDC', async () => {
    ...
    const result = await client.getMarketObservation(AAVE_V3, 'supply_apy', 'USDC');
  ...
  it('extracts the min BORROWER/VARIABLE rate for borrow_apy', async () => {
    ...
    const result = await client.getMarketObservation(AAVE_V3, 'borrow_apy', 'USDC');
  ...
  it('computes utilization as borrow/deposit ratio and picks the max market', async () => {
    ...
    const result = await client.getMarketObservation(AAVE_V3, 'utilization', 'USDC');
  ...
  it('skips markets with zero deposits when computing utilization', async () => {
    ...
    await expect(client.getMarketObservation(AAVE_V3, 'utilization', 'USDC')).rejects.toThrow(
      /No utilization data/,
    );
  });
  ...
  it('filters by asset symbol and fails closed when no market matches', async () => {
    ...
    await expect(client.getMarketObservation(AAVE_V3, 'supply_apy', 'WETH')).rejects.toThrow(
      /No WETH market found for aave-v3/,
    );
  });
  ...
  it('ignores inactive markets during asset filtering', async () => {
      ...
      envelope({
        data: {
          markets: [
            {
              inputToken: { symbol: 'USDC', decimals: 6 },
              rates: [{ rate: '4.75', side: 'LENDER', type: 'VARIABLE' }],
              totalValueLockedUSD: '1250000000',
              totalDepositBalanceUSD: '1000000000',
              totalBorrowBalanceUSD: '784000000',
              indexLastUpdatedTimestamp: '1788825600',
    ...
    await expect(client.getMarketObservation(AAVE_V3, 'supply_apy', 'USDC')).rejects.toThrow(
      /No USDC market found/,
    );
  });
      ...
      envelope({
        data: {
          markets: [
            {
              inputToken: { symbol: 'USDC', decimals: 6 },
              rates: [{ rate: '6.25', side: 'BORROWER', type: 'VARIABLE' }],
              totalValueLockedUSD: '1250000000',
              totalDepositBalanceUSD: '1000000000',
              totalBorrowBalanceUSD: '784000000',
              indexLastUpdatedTimestamp: '1788825600',
...
function snapshot(days: number, rate: string, extra: Record<string, unknown> = {}) {
  const offset = 6 - days;
  return {
    days: String(days),
...
describe('GraphGatewayClient.getMarketHistory', () => {
  ...
  it('returns a cited daily series ordered oldest to newest', async () => {
    const fetchImpl = mockFetch(historyEnvelope(RATES.map((rate, days) => snapshot(days, rate))));
    const client = new GraphGatewayClient({
      apiKey: 'test-graph-key',
      fetchImpl: fetchImpl as typeof fetch,
    });

    const points = await client.getMarketHistory(AAVE_V3, 'supply_apy', 'USDC', 7);

    expect(points).toHaveLength(7);
    expect(points.map((point) => point.days)).toEqual([0, 1, 2, 3, 4, 5, 6]);
    expect(points.map((point) => point.value)).toEqual([3.8, 3.86, 3.95, 4.02, 4.1, 4.18, 4.25]);
  ...
  it('slices the series to the requested window', async () => {
    ...
    const points = await client.getMarketHistory(AAVE_V3, 'supply_apy', 'USDC', 3);
  ...
  it('derives utilization snapshots from borrow/deposit balances', async () => {
    ...
    const points = await client.getMarketHistory(AAVE_V3, 'utilization', 'USDC', 7);
  ...
  it('fails closed when fewer than two usable snapshots survive', async () => {
    ...
    await expect(client.getMarketHistory(AAVE_V3, 'supply_apy', 'USDC', 7)).rejects.toThrow(
      /at least 2 are required for a trend/,
    );
  });
  ...
  it('rejects a window that cannot produce a trend', async () => {
    ...
    await expect(client.getMarketHistory(AAVE_V3, 'supply_apy', 'USDC', 1)).rejects.toThrow(
      /at least 2 days/,
    );
    await expect(client.getMarketHistory(AAVE_V3, 'supply_apy', 'USDC', 7.5)).rejects.toThrow(
      /at least 2 days/,
    );
  ...
  it('ignores inactive markets and fails closed when no market matches the asset', async () => {
    const inactive = mockFetch(historyEnvelope([snapshot(6, '4.25')], { isActive: false }));
    const inactiveClient = new GraphGatewayClient({
      apiKey: 'test-graph-key',
      fetchImpl: inactive as typeof fetch,
    });

    await expect(inactiveClient.getMarketHistory(AAVE_V3, 'supply_apy', 'USDC', 7)).rejects.toThrow(
      /No USDC market found for aave-v3/,
    );
    ...
    await expect(otherClient.getMarketHistory(AAVE_V3, 'supply_apy', 'WETH', 7)).rejects.toThrow(
      /No WETH market found for aave-v3/,
    );
  });
