...
import { ASKCHING_TOOLS, runGrokOrchestrator, type ChatCompletionClient } from './loop.js';
...
describe('runGrokOrchestrator', () => {
  it('executes a model-requested comparison and returns the cited synthesis', async () => {
    ...
    expect(complete.mock.calls[0]?.[0].tools.map((tool) => tool.function.name)).toEqual([
      'get_info',
      'compare_markets',
      'research_brief',
      'risk_scan',
  ...
  it('executes a model-requested cited market analysis', async () => {
    const complete = vi
      ...
      .mockImplementationOnce(async (request) => {
        ...
        expect(
          analysis.findings.some((finding: { severity: string }) => finding.severity === 'high'),
        ).toBe(true);
        return {
          role: 'assistant',
          content: 'Spark has the highest spot utilization, with cited evidence and caveats.',
        };
      });
    ...
    const complete = vi
      ...
      .mockImplementationOnce(async (request) => {
        ...
        return {
          role: 'assistant',
          content:
            'Lending supply APY and LP fee APR are ranked separately with citations and risks; no transaction was proposed.',
        };
      });
  ...
  it('describes discovery as separate category rankings with no transaction execution', () => {
    ...
    const parameters = discovery?.function.parameters as {
      properties?: Record<
        string,
        { enum?: string[]; default?: unknown; items?: { enum?: string[] } }
      >;
    };
  ...
  it('routes a historical question to analyze_trends with a cited series', async () => {
    const complete = vi
      ...
      .mockImplementationOnce(async (request) => {
        ...
        return {
          role: 'assistant',
          content: 'Aave V3 supply APY rose while Compound V3 fell over the cited 7d window.',
        };
      });
  ...
  it('fails closed when analyze_trends cannot assemble two cited series', async () => {
    const complete = vi.fn<ChatCompletionClient['complete']>().mockResolvedValueOnce({
      role: 'assistant',
      content: null,
      tool_calls: [
        {
