 ...
 */
import { createMarketDataSource, type MarketDataSource } from '../packages/shared/src/index.js';
...
const question =
  cleanArgs.join(' ').trim() || 'Compare USDC supply APY across Aave V3 and Compound V3';
...
for (const figure of result.brief.keyFigures) {
  console.log(`  ${figure.protocol}: ${figure.value}% (${figure.unit}) [source: ${figure.source}]`);
}
console.log('');
...
for (const source of result.sources) {
  console.log(`  ${source.protocol}: subgraph ${source.subgraphId} (block ${source.block})`);
}
console.log('');
console.log('Caveats:');
