...
const compound = LIVE_SOURCES.find(
  (source) => source.protocol === "compound-v3"
);
if (!aave || !compound) {
  throw new Error(
    "live-smoke requires aave-v3 and compound-v3 in LIVE_SOURCES"
  );
}
...
for (const metric of ["utilization", "borrow_apy"]) {
  const covered =
    results.some((result) => result.metric === metric) ||
    gaps.some((gap) => gap.metric === metric);
  if (!covered) {
    throw new Error(
      `live-smoke FAIL: no live or gapped coverage for ${metric}`
    );
  }
}
console.log(
  "live-smoke coverage: utilization + borrow_apy + multi-asset rows exercised"
);
