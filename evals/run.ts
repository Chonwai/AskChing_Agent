...
type EvalKind =
  ...
  | 'analyze_trends'
  | 'discover_yields';
type EvalProtocol = 'aave-v3' | 'compound-v3' | 'spark-lend' | 'aave-v2' | 'uwu-lend' | 'zerolend';
...
for (const testCase of cases) {
  ...
  if (kind === 'compare_markets') {
    ...
    assert(
      typeof result.brief.conclusion === 'string' && result.brief.conclusion.length > 0,
      testCase.id,
      'expected a non-empty brief conclusion',
    );
    assert(result.brief.keyFigures.length >= 2, testCase.id, 'expected at least two key figures');
    assert(
    ...
    if (testCase.input.asset) {
      assert(
        result.brief.keyFigures.every((figure) => figure.metric === testCase.input.metric),
        testCase.id,
    ...
    assert(result.sources.length >= 2, testCase.id, 'expected at least two sources');
    assert(
      result.sources.every(
        (source) =>
          Boolean(source.subgraphId) && Boolean(source.timestamp) && Boolean(source.queryHash),
      ),
      testCase.id,
    ...
    assert(Array.isArray(result.gaps), testCase.id, 'expected gaps to be an array');
    assert(
      result.gaps.every(
        (gap) =>
    ...
    result.findings
      ...
      .forEach((citation) => sourceIds.add(citation.subgraphId));
    ...
    assert(!('winner' in result), testCase.id, 'expected no combined lending/LP winner field');
...
function assert(condition: unknown, caseId: string, message: string): asserts condition {
  if (!condition) {
