...
| **6** | MCP Server Smoke（跨平台證明） | ❌（fixture mode） | 1 分鐘 |

> **總計：** 完整旅程約 **17 分鐘**；只跑「快速完成清單」（Station 0/1/2/4/6）約 **10 分鐘**。**全程不需要任何視覺化工具，全部在終端機完成。**
...
> 目標：證明 AskChing 是標準 MCP server —— 通過 stdio handshake（`initialize` → `notifications/initialized` → `tools/list`），對任何 MCP client（Cursor / Claude / Codex / Gemini）皆可用。此站以 fixture mode（`DEMO_LIVE=0`）執行，**不需 API key**。
...
```
mcp-smoke OK: askching (3 tools)
```

### ✅ 驗證點

- [ ] 輸出為 `mcp-smoke OK: askching (3 tools)`（或含相同字樣）
- [ ] 列出 3 個 tools：`compare_markets` / `research_brief` / `risk_scan`
- [ ] stdio handshake 成功（`initialize` + `notifications/initialized` 無 timeout）
- [ ] exit code 為 0

> 🔌 **跨平台意義：** 三 tools 暴露的正是同一組 Grok 可呼叫的函數（`packages/mcp-server/src/tools.ts`）。任何支援 MCP 的 agent 都可直接掛載此 server —— 這是「cross-platform agent 入口」的證明。
